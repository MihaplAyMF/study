...
ParticleNode::ParticleNode(Particle::Type type, const TextureHolder& textures)
	: SceneNode()
	, mParticles()
	, mTextures(textures.get(Textures::Particle))
	, mType(type)
...
{
	sf::Vector2f size(mTextures.getSize());
	...
	for(const Particle& particle : mParticles)
	{
		...
		float ration = particle.lifetime.asSeconds() / Table[mType].lifetime.asSeconds();
		color.a = static_cast<sf::Uint8>(255 * std::max(ration, 0.f));
